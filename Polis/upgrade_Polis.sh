...
# Upgrade Sentinel if necessary
printf "\n\e[93mUpdating sentinel...\e[0m\n"
cd /home/${daemon_user}/sentinel/ && sudo -H -u ${daemon_user} bash -c "git pull"
...
sudo -H -u ${daemon_user} bash -c "polis-cli -conf=/etc/polis/polis.conf getinfo"
sudo -H -u ${daemon_user} bash -c "polis-cli -conf=/etc/polis/polis.conf masternode status"
