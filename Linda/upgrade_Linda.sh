...
until sudo -H -u $COIN_user bash -c "COIN-cli getinfo" &>/dev/null; do
  sleep 2
done

sudo -H -u $COIN_user bash -c "COIN-cli getinfo"
sudo -H -u $COIN_user bash -c "COIN-cli masternode status"
